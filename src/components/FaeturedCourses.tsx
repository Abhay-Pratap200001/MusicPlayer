            ...
            <div key={course.id} className="flex justify-center">
              <BackgroundGradient className="flex flex-col rounded-[22px] bg-white dark:bg-zinc-900 overflow-hidden h-max-s w-screen]">
                <div className="p-4 sm:p-6 flex flex-col items-center text-center flex-grow">
                  ...
                  <p className="text-lg sm:text-2xl tracking-tight font-semibold text-black mt-3 mb-2 dark:text-neutral-200">
                    {course.title}
                  </p>

                  <p className="text-sm text-neutral-400 flex-grow py-3">{course.description}</p>

                  <Link href={`/courses/${course.slug}`} className="py-3">Learn More</Link>
                </div>
              </BackgroundGradient>
            </div>
          ))}
...
export default FaeturedCourses;
